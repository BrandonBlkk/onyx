...
const ActionCard = ({ title, description, icon: Icon, badge, isDark, viewMode }) => {
  if (viewMode === 'list') {
    return (
      <article
        className={`group relative grid overflow-hidden border backdrop-blur-xl sm:grid-cols-[160px_minmax(0,1fr)] ${getInteractivePanelClass(isDark)}`}
      >
        <div
        ...
        <div
          className={`relative flex min-h-37 items-center justify-center border-b p-4 sm:border-r sm:border-b-0 ${
            isDark
  ...
  return (
    <article
      className={`group relative overflow-hidden rounded-3xl border backdrop-blur-xl ${getInteractivePanelClass(isDark)}`}
    >
      <div
        className={`absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100 ${
          isDark ? 'bg-accent-500/3' : 'bg-accent-500/5'
        ...
        <div>
          <span
            className={`rounded-full border px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.16em] select-none ${
              isDark
                ? 'border-zinc-800 bg-zinc-950/70 text-zinc-400'
                : 'border-slate-200 bg-white/85 text-slate-500 shadow-sm'
            }`}
          >
            {badge}
          </span>

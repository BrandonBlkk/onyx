...
import { MoreHorizontal, Pencil, Heart, Trash2, Info, Download, Copy, Type } from 'lucide-react'

const ResumeCardActions = ({ isDark, onRename }) => {
  ...
  const menuItems = [
    { icon: <Pencil className="h-4 w-4" />, className: `${menuItemClass}`, label: 'Edit' },
    { icon: <Type className="h-4 w-4" />, className: `${menuItemClass}`, label: 'Rename', onClick: onRename },
    { icon: <Info className="h-4 w-4" />, className: `${menuItemClass}`, label: 'Details' },
    { icon: <Download className="h-4 w-4" />, className: `${menuItemClass}`, label: 'Download' },
    { icon: <Copy className="h-4 w-4" />, className: `${menuItemClass}`, label: 'Make a copy' },
    { icon: <Trash2 className="h-4 w-4" />, className: `${menuItemClass} hover:text-red-500! hover:bg-red-100!`, label: 'Move to trash' },
  ]
    ...
    <div className="invisible absolute right-3 top-3 z-20 flex translate-y-1 scale-95 items-start gap-1 opacity-0 transition-all duration-200 ease-out group-hover:visible group-hover:translate-y-0 group-hover:scale-100 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:scale-100 group-focus-within:opacity-100">
      ...
      <div className="group/menu relative">
        ...
        <div role="menu" className={menuClass}>
          {menuItems.map(({ icon, className, label, onClick }) => {
            return (
              <button
                key={label}
                type="button"
